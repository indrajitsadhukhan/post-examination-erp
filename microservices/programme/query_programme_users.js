...
const getProgrammeUserById = (request, response) => {
    ...
    client.query('SELECT first_name, programme.name , programme.id FROM programme_users,users,programme_versions,programme WHERE users.id = $1 AND programme_users.user_id = users.id AND programme_users.programme_id=programme_versions.id AND programme_versions.programme_id = programme.id', [id], (error, results) => {
        if(error)
        {
...
//It makes no sense to give end date now
    ...
    client.query('INSERT INTO programme_users (programme_id,user_id,start_date) VALUES ($1,$2,$3) RETURNING id', [ prog_id, user_id, start_date ], (error,results) => {
        if(error) {
            response.status(500).send({ error });
        }
...
const updateProgrammeUserEndDate = (request,response) => {
    ...
    client.query('UPDATE programme_users SET end_date = $1 WHERE user_id = $2',[end_date,id],(error,results) => {
        if(error)
        {
            response.status(500).send({ error });
        }
